...
class ClientSocket {
    ...
    private onConnect(e: egret.Event): void {
        ...
        EventMananger.Instance.sendEvent(EventConst.SocketConnect, this);
        this.sendHeart();
    }
    ...
    private tryReconnect() {
        ...
        if (this.curReconnectCount <= this.reconnenctLimit) {
            ...
            if (this.curReconnectCount == 1) {
                EventMananger.Instance.sendEvent(EventConst.StartReconnect, this);
            }
            //开始断线重连
    ...
    public send(proto: string, data: any = {}) {
        if (this.socket && this.socket.connected) {
            ...
            EventMananger.Instance.sendEvent(EventConst.SocketNotConnect, this);
            EventMananger.Instance.sendEvent(EventConst.SocketClose, this);
        }
    }
